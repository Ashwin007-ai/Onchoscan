...
(function () {
  ...
  function initDanger(profile) {
    ...
    // clear profile data (server-side reset of optional fields)
    var clearBtn = $id("clearBtn");
    if (clearBtn) clearBtn.addEventListener("click", async function() {
      if (!confirm("This will reset your avatar, bio, phone, location and all optional profile fields. Your account and scan history will not be affected. Continue?")) return;
      clearBtn.textContent = "Clearing…";
      clearBtn.disabled = true;
      try {
        var r = await fetch(API + "/profile/clear", {
          method: "POST",
          headers: authHdr()
        });
        if (!r.ok) throw new Error("Server error " + r.status);
        toast("Profile data cleared ✓", "ok");
        // Reload page after short delay so all fields refresh
