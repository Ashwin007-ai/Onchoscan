...
def clear_profile_data(current_user: dict = Depends(get_current_user)):
    ...
    conn.execute("""
        UPDATE users SET
            phone = '', dob = '', gender = '', role = '', org = '',
            city = '', country = '', bio = '', specs = '', avatar = '',
            prefs = '{}'
        WHERE username = ?
    """, (current_user["username"],))
    conn.commit()
        ...
        try:
            ...
            conn.execute(
                ...
                (str(uuid.uuid4()), current_user["username"],
                 ...
                 cancer_type, result["prediction"], result["confidence"],
                 result["risk_level"], report_path,
                 patient_name, patient_age, patient_sex, patient_symptoms, patient_note, "batch")
            )
            conn.commit()
...
def delete_history(ids: list[str], current_user: dict = Depends(get_current_user)):
    ...
    params = ids + [current_user["username"]]
    conn.execute(
        f"DELETE FROM predictions WHERE id IN ({placeholders}) AND username=?",
        params
    )
    conn.commit()
    conn.close()
...
def analytics(current_user: dict = Depends(get_current_user)):
    ...
    username = current_user["username"]
    rows = conn.execute(
        "SELECT * FROM predictions WHERE username=? ORDER BY timestamp ASC",
        (username,)
    ).fetchall()
    conn.close()
    ...
    for i in range(11, -1, -1):
        m = today.replace(day=1) - dt.timedelta(days=i * 30)
        key = m.strftime("%Y-%m")
    ...
    for s in scans:
        ...
        if c2 < 50:   conf_buckets["<50%"] += 1
        elif c2 < 65: conf_buckets["50-65%"] += 1
        elif c2 < 80: conf_buckets["65-80%"] += 1
    ...
    return {
        "total": total,
        "empty": False,
        "by_cancer_type": {"brain": brain, "skin": skin},
        "by_risk": {"high": high, "medium": medium, "low": low},
        "by_analysis_type": {"single": single_count, "batch": batch_count},
        ...
        "top_patients": [{"name": n, "count": cnt} for n, cnt in top_patients],
...
def get_patients(current_user: dict = Depends(get_current_user)):
    conn = get_db()
    rows = conn.execute(
        """SELECT DISTINCT patient_name FROM predictions
           WHERE username=? AND patient_name IS NOT NULL AND patient_name != ''
...
def get_patient_scans(patient_name: str, current_user: dict = Depends(get_current_user)):
    ...
    rows = conn.execute(
        """SELECT * FROM predictions
           WHERE username=? AND patient_name=?
           ORDER BY timestamp ASC""",
        (current_user["username"], patient_name)
    ).fetchall()
    conn.close()
